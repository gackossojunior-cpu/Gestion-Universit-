...
def require_rh(request):
    ...
    if request.user.is_superuser or request.user.groups.filter(name='RH').exists():
        return None
    return Response({'detail': 'Accès réservé aux Ressources Humaines.'}, status=status.HTTP_403_FORBIDDEN)
...
def rh_dashboard(request):
    ...
    if annee_active:
        ens_actifs_annee = ens_qs.filter(annee_academique=annee_active, actif=True)
    else:
    ...
    return Response({
        ...
        'personnel_par_fonction': list(
            ...
            .values('fonction').annotate(n=Count('id')).order_by('-n')
        ),
        'total_enseignants_actifs': ens_actifs_annee.count(),
        'enseignants_vacataires': ens_actifs_annee.filter(statut='vacataire').count(),
        'enseignants_permanents': ens_actifs_annee.filter(statut='permanent').count(),
        'chefs_departement_actifs': ens_actifs_annee.filter(chef_departement=True).count(),
...
def rh_enseignants(request):
    ...
    qs = Enseignant.objects.select_related('faculte', 'filiere', 'annee_academique')
    if annee_active:
        qs = qs.filter(annee_academique=annee_active)
    if statut in ('vacataire', 'permanent'): qs = qs.filter(statut=statut)
    if actif_only: qs = qs.filter(actif=True)
    if chef_only: qs = qs.filter(chef_departement=True)
    if query:
        qs = qs.filter(
            Q(nom__icontains=query) | Q(prenom__icontains=query) |
            Q(matricule__icontains=query) | Q(email__icontains=query)
        )
    # Un permanent est filtré sur son unique faculté/filière ; un vacataire
    # est filtré sur ses affectations (il peut en avoir plusieurs).
    if fac_id:
    ...
    if enseignants and annee_active:
        for it in InterventionEnseignant.objects.filter(
            enseignant__in=enseignants, annee_academique=annee_active
        ):
            interventions_map.setdefault(it.enseignant_id, []).append(it.semestre)
    ...
    if vacataires:
        for a in AffectationVacataire.objects.filter(enseignant__in=vacataires).select_related('faculte', 'portail', 'filiere'):
            affectations_map.setdefault(a.enseignant_id, []).append(_serialize_affectation(a))
        ...
        'enseignants': [
            {
                ...
                'genre': e.genre, 'email': e.email, 'telephone': e.telephone,
                'faculte_code': e.faculte.code if e.faculte else None,
                'faculte_nom': e.faculte.nom if e.faculte else None,
                ...
                'actif': e.actif,
                'date_debut_contrat': e.date_debut_contrat.isoformat() if e.date_debut_contrat else None,
                'date_fin_contrat': e.date_fin_contrat.isoformat() if e.date_fin_contrat else None,
                'contrat_expire': e.contrat_expire(),
                'taux_horaire': str(e.taux_horaire) if e.taux_horaire is not None else None,
                'salaire_mensuel': str(e.salaire_mensuel) if e.salaire_mensuel is not None else None,
                'interventions': sorted(interventions_map.get(e.id, [])),
...
def rh_ajouter_enseignant(request):
    ...
    try:
        ...
        chef_departement = bool(body.get('chef_departement'))

        statut = body.get('statut', '')
        date_debut_contrat = body.get('date_debut_contrat') or None
        date_fin_contrat = body.get('date_fin_contrat') or None
        taux_horaire = body.get('taux_horaire') or None
        salaire_mensuel = body.get('salaire_mensuel') or None
        affectations = body.get('affectations', [])  # vacataire uniquement

        # ── Champs communs obligatoires ──
        if not all([nom, prenom, genre, email, annee_id, statut, date_debut_contrat]):
            return Response({'detail': 'Tous les champs obligatoires doivent être remplis.'},
                             status=status.HTTP_400_BAD_REQUEST)
        if statut not in ('vacataire', 'permanent'):
            return Response({'detail': 'Statut invalide.'}, status=status.HTTP_400_BAD_REQUEST)
        if Enseignant.objects.filter(email=email).exists():
            return Response({'detail': f"Un enseignant avec l'email {email} existe déjà."},
                             status=status.HTTP_400_BAD_REQUEST)

        # ── Logique intelligente selon le statut ──
        # Permanent : une seule faculté/filière fixe, salaire mensuel obligatoire.
        # Vacataire : pas de faculté/filiere fixe — au moins UNE affectation
        #             (faculté + semestre + volume horaire ; filière facultative
        #             si le cours concerne toute la faculté), taux horaire obligatoire.
        faculte_id, filiere_id = None, None
        if statut == 'permanent':
            faculte_id = body.get('faculte')
            filiere_id = body.get('filiere') or None
            if not faculte_id:
                return Response({'detail': "La faculté est obligatoire pour un permanent."},
                                 status=status.HTTP_400_BAD_REQUEST)
            if not salaire_mensuel:
                return Response({'detail': "Le salaire mensuel est obligatoire pour un permanent."},
                                 status=status.HTTP_400_BAD_REQUEST)
            taux_horaire = None
        else:  # vacataire
            if not taux_horaire:
                return Response({'detail': "Le taux horaire est obligatoire pour un vacataire."},
                                 status=status.HTTP_400_BAD_REQUEST)
            if not date_fin_contrat:
                return Response({'detail': "La date de fin de contrat est obligatoire pour un vacataire."},
                                 status=status.HTTP_400_BAD_REQUEST)
            if not affectations:
                return Response({'detail': "Ajoutez au moins une affectation (faculté, semestre, volume horaire)."},
                                 status=status.HTTP_400_BAD_REQUEST)
            for a in affectations:
                if not a.get('faculte') or not a.get('semestre') or not a.get('volume_horaire_prevu'):
                    return Response({'detail': "Chaque affectation doit avoir une faculté, un semestre et un volume horaire."},
                                     status=status.HTTP_400_BAD_REQUEST)
                if a.get('portail') and a.get('filiere'):
                    return Response({'detail': "Une affectation ne peut pas avoir à la fois un portail ET une filière — choisissez l'un ou l'autre."},
                                     status=status.HTTP_400_BAD_REQUEST)
                # Un portail n'existe qu'en S1-S2, une filière qu'à partir de S3 —
                # on refuse toute incohérence plutôt que de laisser un mauvais
                # choix passer silencieusement.
                sem_num = int(a['semestre'].replace('S', ''))
                if a.get('portail') and sem_num > 2:
                    return Response({'detail': f"Un portail ne concerne que S1-S2 (affectation en {a['semestre']})."},
                                     status=status.HTTP_400_BAD_REQUEST)
                if a.get('filiere') and sem_num <= 2:
                    return Response({'detail': f"Une filière ne concerne qu'à partir de S3 (affectation en {a['semestre']})."},
                                     status=status.HTTP_400_BAD_REQUEST)
        ...
        with transaction.atomic():
            ens = Enseignant(
                ...
                lieu_naissance=lieu_naissance or None, nationalite=nationalite,
                email=email, telephone=telephone or None, adresse=adresse or None, rib=rib or None,
                numero_secu=numero_secu or None, diplome=diplome or None,
                faculte_id=faculte_id, filiere_id=filiere_id, annee_academique_id=annee_id,
                chef_departement=chef_departement, statut=statut,
                date_debut_contrat=date_debut_contrat, date_fin_contrat=date_fin_contrat,
                taux_horaire=taux_horaire, salaire_mensuel=salaire_mensuel,
                actif=True,
            )
            ens.save()
            ...
            if statut == 'vacataire':
                AffectationVacataire.objects.bulk_create([
                    AffectationVacataire(
                        ...
                        portail_id=a.get('portail') or None,
                        filiere_id=a.get('filiere') or None,
                        semestre=a['semestre'],
                        volume_horaire_prevu=a['volume_horaire_prevu'],
                    )
                    for a in affectations
                ])

        return Response({
            'detail': f"{nom} {prenom} enregistré(e) avec le matricule {ens.matricule}.",
            'matricule': ens.matricule, 'id': ens.id,
        })
...
@api_view(['POST'])
def rh_changer_statut_enseignant(request, enseignant_id):
    ...
    body = request.data
    if 'actif' in body:
        ens.actif = bool(body.get('actif'))
    if 'date_fin_contrat' in body:
        ens.date_fin_contrat = body.get('date_fin_contrat') or None
    ens.save(update_fields=['actif', 'date_fin_contrat'])
    return Response({'detail': "Enseignant mis à jour.", 'actif': ens.actif})


@api_view(['POST'])
def rh_modifier_volume_affectation(request, affectation_id):
    ...
    try:
        a = AffectationVacataire.objects.select_related('enseignant', 'faculte', 'filiere').get(id=affectation_id)
    except AffectationVacataire.DoesNotExist:
        return Response({'detail': 'Affectation introuvable.'}, status=status.HTTP_404_NOT_FOUND)

    nouveau_volume = request.data.get('volume_horaire_prevu')
    motif = request.data.get('motif', '').strip()
    if not nouveau_volume:
        return Response({'detail': 'Le nouveau volume horaire est requis.'}, status=status.HTTP_400_BAD_REQUEST)

    ancien_volume = a.volume_horaire_prevu
    with transaction.atomic():
        HistoriqueVolumeHoraire.objects.create(
            affectation=a, ancien_volume=ancien_volume,
            nouveau_volume=nouveau_volume, motif=motif,
        )
        a.volume_horaire_prevu = nouveau_volume
        a.save(update_fields=['volume_horaire_prevu'])

    return Response({
        'detail': f"Volume horaire mis à jour : {ancien_volume}h → {nouveau_volume}h.",
        'volume_horaire_prevu': str(a.volume_horaire_prevu),
    })
...
def rh_personnel(request):
    ...
    qs = Personnel.objects.select_related('faculte_rattachement')
    if fonction: qs = qs.filter(fonction=fonction)
    if statut: qs = qs.filter(statut=statut)
    if fac_id: qs = qs.filter(faculte_rattachement_id=fac_id)
    if query:
        qs = qs.filter(
            Q(nom__icontains=query) | Q(prenom__icontains=query) |
            Q(matricule__icontains=query) | Q(telephone__icontains=query)
        )
        ...
        'personnel': [
            {
                ...
                'genre': p.genre, 'telephone': p.telephone, 'email': p.email,
                'fonction': p.fonction, 'fonction_display': p.get_fonction_display_full(),
                'lieu_travail': p.lieu_travail,
                'faculte_nom': p.faculte_rattachement.nom if p.faculte_rattachement else None,
                'date_embauche': p.date_embauche.isoformat() if p.date_embauche else None,
                'salaire_mensuel': str(p.salaire_mensuel) if p.salaire_mensuel is not None else None,
                'statut': p.statut,
            }
            for p in qs
        ],
...
def rh_ajouter_personnel(request):
    ...
    try:
        ...
        fonction_autre = body.get('fonction_autre', '').strip()
        lieu_travail = body.get('lieu_travail', '').strip()
        faculte_id = body.get('faculte_rattachement') or None
        date_embauche = body.get('date_embauche', '')
        salaire_mensuel = body.get('salaire_mensuel') or None
        rib = body.get('rib', '').strip()

        if not all([nom, prenom, genre, fonction, lieu_travail, date_embauche, salaire_mensuel]):
            return Response({'detail': 'Tous les champs obligatoires doivent être remplis (le salaire mensuel est obligatoire pour la transmission à la Finance).'},
                             status=status.HTTP_400_BAD_REQUEST)
        if fonction == 'autre' and not fonction_autre:
            return Response({'detail': "Précisez la fonction pour 'Autre'."},
                             status=status.HTTP_400_BAD_REQUEST)

        p = Personnel(
            nom=nom, prenom=prenom, genre=genre, telephone=telephone or None, email=email,
            fonction=fonction, fonction_autre=fonction_autre or None,
            lieu_travail=lieu_travail, faculte_rattachement_id=faculte_id,
            date_embauche=date_embauche, salaire_mensuel=salaire_mensuel, rib=rib or None,
            statut='actif',
        )
        p.save()
        return Response({'detail': f"{nom} {prenom} enregistré(e) avec le matricule {p.matricule}.",
                          'matricule': p.matricule, 'id': p.id})
    except Exception as e:
        return Response({'detail': f"Erreur lors de l'enregistrement : {e}"},
                         status=status.HTTP_400_BAD_REQUEST)
...
@api_view(['POST'])
def rh_changer_statut_personnel(request, personnel_id):
    err = require_rh(request)
    if err: return err
    ...
    return Response({'detail': f"Statut mis à jour : {nouveau_statut}", 'statut': nouveau_statut})
...
#                 'email': e.email, 'genre': e.genre, 'age': e.get_age(),
#                 'semestre': e.semestre, 'cycle': e.cycle, 'statut': e.statut,
#                 'faculte_code': e.faculte.code if e.faculte else None,
