...
def etu_dashboard(request):
    ...
    # Programme du semestre actuel (toujours visible)
    if etu.semestre in ['S1', 'S2']:
        unites = UniteEnseignement.objects.filter(
            semestre=etu.semestre,
            portail__faculte=etu.faculte
        ).prefetch_related('matieres').order_by('code_ue')
    else:
        unites = UniteEnseignement.objects.filter(
            semestre=etu.semestre,
            filiere__portail__faculte=etu.faculte
        ).prefetch_related('matieres').order_by('code_ue')

    # Résultats : semestres passés + semestre actuel si en règle
    ...
    for sem in semestres_notes:
        # Semestre actuel : visible seulement si en règle
        if sem == etu.semestre and not etu.en_regle:
            continue
        structure, _ = calculer_matrice(
            None, sem, etudiant_id=etu.id, save_results=False
        )
        if structure:
            resultats[sem] = structure[0]
