...
from .decorators import enseignant_required
...
# ── DASHBOARD ─────────────────────────────────────────────────
@enseignant_required
def ens_dashboard(request):
    return render(request, 'gestion_eleves/enseignant/dashboard.html', {
...
def ens_liste_classe(request):
    ...
    return render(request, 'gestion_eleves/enseignant/liste_classe.html', {
...
def ens_saisie_notes(request):
    ...
    if fac_id and sem:
        if sem in ['S1', 'S2']:
            unites = list(UniteEnseignement.objects.filter(
                semestre=sem, portail__faculte_id=fac_id).order_by('code_ue'))
        else:
            unites = list(UniteEnseignement.objects.filter(
                semestre=sem, filiere__portail__faculte_id=fac_id).order_by('code_ue'))
    ...
    return render(request, 'gestion_eleves/enseignant/saisie_notes.html', {
...
# ── NOTE INDIVIDUELLE ─────────────────────────────────────────
@enseignant_required
def ens_note_individuelle(request, etudiant_id):
    ...
    if request.method == 'POST':
        ...
        return redirect('ens_note_individuelle', etudiant_id=etudiant_id)

    return render(request, 'gestion_eleves/enseignant/note_individuelle.html', {
...
def ens_deliberation(request):
    ...
    return render(request, 'gestion_eleves/enseignant/deliberation.html', {
...
def ens_matrice_a3(request):
    ...
    response['Content-Disposition'] = (
        f'inline; filename="MatriceA3_{faculte.code}_{sem_id}.pdf"'
    )
